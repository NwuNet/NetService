...
class DatabaseModel {
	...
	public $indexfile = './Backup/index.html';
    ...
    public function backupList()
    {
        ...
        $fileDir = $this->backupDir . DIRECTORY_SEPARATOR;
        if(!is_dir($fileDir)){
            return false;
        }
        ...
        if(is_array($listFile)){
            ...
            foreach ($listFile as $key => $value) {
                ...
                $list[$key]['time'] = $fileName[0].'_'.$fileName[1];
				$list[$key]['day'] = $fileName[0];
				$list[$key]['sec'] = $fileName[1];
            }
        }
        return $list;
    }
    ...
    public function importData($time){
        ...
        foreach($files as $name){
            ...
            $match = sscanf($basename, '%4s%2s%2s_%2s%2s%2s-%d');
            $gz = preg_match('/^\d{8,8}_\d{6,6}_\d+\.sql.gz$/', $basename);
